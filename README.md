...
#### Do you actually need config.yaml?

**No, not for a basic single-node setup.**
k3s works perfectly fine without it — this server proves that.

k3s has sensible built-in defaults for most options, so `config.yaml` is optional.
You only need it when the defaults are not enough for your situation.

| Situation                                                         | Need config.yaml? | Why                                                                                              |
| ----------------------------------------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------ |
| Simple local cluster, kubectl via `sudo`                          | No                | Defaults work fine                                                                               |
| Want to run `kubectl` without `sudo`                              | Yes               | Set `write-kubeconfig-mode: "0644"` so `/etc/rancher/k3s/k3s.yaml` is readable by non-root users |
| Accessing the API from a public domain (e.g. `k8s.bit-habit.com`) | Yes               | Add `tls-san` so the API server certificate includes that domain name                            |
| Want to replace the built-in load balancer                        | Yes               | Set `disable: [servicelb]`                                                                       |
| Multi-node cluster with specific node roles                       | Yes               | Use `node-label` to identify nodes for scheduling                                                |

#### What is `/etc/rancher/k3s/k3s.yaml`?

This is a **separate, auto-generated file** — not the same as `config.yaml`.

k3s creates it automatically at startup. It is a kubeconfig file: it contains the cluster address,
the TLS certificate, and the client credentials kubectl needs to connect.

```
/etc/rancher/k3s/config.yaml   ← you write this (optional, controls k3s startup behaviour)
/etc/rancher/k3s/k3s.yaml      ← k3s writes this (auto-generated, used by kubectl)
```

By default `k3s.yaml` has permissions `0600` (root-only).
On this server, a copy was made to `/home/ubuntu/.kube/config` at some point,
which is why `kubectl` works for the `ubuntu` user without `sudo`.

If you had `write-kubeconfig-mode: "0644"` in `config.yaml`, k3s would generate
`k3s.yaml` as world-readable from the start, and no manual copy would be needed.

#### Why this is the first file to read

`config.yaml` (if it exists) is the foundation. Everything else (Traefik, the cluster API,
the kubeconfig) is generated by k3s based on this configuration.
On this server it does not exist, so k3s runs on defaults — which is fine for now.
